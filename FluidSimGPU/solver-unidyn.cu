...
const int nspts = 5000; //number of solid particles
...
int main(int argc, char **argv)
{
    ...
    for (dev = 0; dev < deviceCount; dev++){
		...
		cudaMallocManaged(&d_m[dev], NUMCELLS * sizeof(float));
	...
	const char * const varnames3[] = { "density", "boundary" };
    ...
    for (dev = 0; dev < deviceCount; dev++){
	    ...
	    for (int j = 0; j < nspts; j++) {
		    	SPptr[dev].push_back(Particle(-.76 + 0.05*((j / 20) % 20), -0.76 + 0.05*(j % 20), -0.60 +(j / 20 /20)*0.05, 0., 0., 0.));
		    	SPptr[dev][j].index = j;
		    	SPptr[dev][j].solid = 0;
		    	SPptr[dev][j].fluid = 1;
	...
	*newsize[1] = dsz[1];
	...
	for (int t = 0; t < tpts; t++) {
        ...
        for (dev = 0; dev < deviceCount; dev++){
			...
			//thrust::device_ptr<int> t_b(d_particleindex[dev]); //not compatible with particle merging yet
		...
		for (dev = 0; dev < deviceCount; dev++){
			cudaSetDevice(dev);
		force_calc_coarse <<<NUMCELLS, 256 >> > (d_SPptr[dev], d_particleindex[dev], v_d[dev], d_start[dev], d_end[dev], d_split[dev], dsz[dev],Ns[dev]+buffer,dev,buffer,d_numsplit[dev]);
			//cudaDeviceSynchronize();
			if (strcmp(cudaGetErrorName(cudaGetLastError()),"cudaSuccess")!=0){
		...
		for (dev = 0; dev < deviceCount; dev++){
			...
			if (numsplit[dev][0]>0){
				force_calc_fine <<<numsplit[dev][0]*8, 256 >> > (d_SPptr[dev], d_particleindex[dev], v_d[dev], d_start[dev], d_end[dev], d_split[dev], dsz[dev],Ns[dev]+buffer,dev,buffer,d_numsplit[dev]);
				cudaDeviceSynchronize();
				if (strcmp(cudaGetErrorName(cudaGetLastError()),"cudaSuccess")!=0){
		...
		//Find max vel
		for (dev = 0; dev< deviceCount; dev++){
			VecMax<<<NUMCELLS, 1024 >>>(d_SPptr[dev], d_particleindex[dev], d_m[dev], dsz[dev], NUMCELLS);
			float temp;
			cudaMemcpy(&temp, d_m[dev], sizeof(float), cudaMemcpyDeviceToHost);
			simTime+=DT*VMAX/max(VMAX/DTMULTMAX,temp)*1000.0;
			//std::cout << "Max velocity: " << d_m[dev][0] <<"\n";
		}
		if (t%40 == 0){
			std::cout << "Time elapsed in simulation: " << simTime << "\n";
		}
		//Update particles
		for (dev = 0; dev < deviceCount; dev++){
			cudaSetDevice(dev);
			update_all << <NUMCELLS, 1024 >> > (d_SPptr[dev], d_particleindex[dev], v_d[dev], d_start_copy[dev], d_start[dev], d_end[dev],d_split[dev],d_numsplit[dev],dsz[dev], Ns[dev]+buffer, d_m[dev], dev, buffer, t, spts[dev], a3[dev], b3[dev]);
			if (strcmp(cudaGetErrorName(cudaGetLastError()),"cudaSuccess")!=0){
		...
		if (t %40 ==0)
			std::cout << "\nElapsed kernel time: " << realTime << " ms\n";
		...
		//Split particles on surface and increase size of particle list up to maximum of nspts+nbpts
		for (dev = 0; dev < deviceCount; dev++){
			cudaSetDevice(dev);
			cudaMemcpy(&SPptr[dev][0], d_SPptr[dev], dsz[dev] * sizeof(Particle), cudaMemcpyDeviceToHost); //copy updated particles back to cpu
